...
// Solução:
function retornaParamsSolucao(params) {
    return params;
}
...
function retornaParamsSolucaoRecriada(params) {
    return params;
}
console.log(retornaParamsSolucaoRecriada("Hogwarts").toLowerCase);
...
function firstFive2(lista) {
    return lista.slice(0, 5);
}
Array; //interface Array<T> - comand + clique na Array para ver
//por isso temos que isso:
const numerosAleatorios = [23, 24, 21, 22];
// É a mesma coisa que isso:
const numerosAleatorios2 = [23, 24, 21, 22]; //interface Array<T>
