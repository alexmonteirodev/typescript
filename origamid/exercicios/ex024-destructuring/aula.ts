...
//caso sejam muitas props, pode passar normalmente em uma interface:
interface Desestruturar {
  nome: string;
  preco?: number;
}

function handleDesestruturarEmInterface({ nome, preco }: Desestruturar) {
  nome.toLowerCase;
  preco?.toFixed;
}
...
function handleClickBtnDeses({ currentTarget }: MouseEvent) {
  console.log("clicou");
}
